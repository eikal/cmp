...
const InitContainerTab = (props) => {
    ...
    const containers = props.podMetadata?.initContainers && props.podMetadata?.initContainers.length > 0 ? <div>
        {
            props.podMetadata?.initContainers.map((nestedObj, key) => {
                return (
                    <div key={nestedObj.name}>
                        <ListItem button onClick={() => handleClick(nestedObj.name)} divider={key < props.podMetadata?.initContainers.length - 1}>
...
export default InitContainerTab;
