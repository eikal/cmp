...
const port = process.env.ENTITIES_API_PORT || 3004;
...
app.listen(port, async () => {
    try {
        console.log(`Agent service is up: ${new Date().toLocaleString()}, on port : ${port}`);
        await getDbConnection();
        cron.schedule(CronJobTimers.AGENT, async () => {
            console.log(`Agent service executed: ${new Date().toLocaleString()}`);
            startSync();
        });
    } catch (error) {
